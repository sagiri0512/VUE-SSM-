        ...
        <div class="table">
          <div class="table-row header">
            <div class="table-cell">
              <input type="checkbox" @click="Allcheckbox()"  v-bind:checked="isAllchecked"  @change="getInfo()" />全选
            </div>
          ...
          <div v-for="item in shoppingList" :key="item.pid" class="table-row">
            <div class="table-cell">
              <input type="checkbox" style="margin-left: -28px" v-model="item.isChecked" @change="Checkbox(), getInfo()" />
            </div>
            <div class="table-cell">
              <img :src="item.pImg" class="productImg" @click="toPage(item.pid)" />
            </div>
            ...
            <div class="table-cell">
              <button class="btn transparent-btn" @click.prevent="decrement(item)">
                -
              </button>
              <div class="table-cell">{{ item.sNum }}</div>
              <button class="btn transparent-btn" @click.prevent="increment(item)">
                +
              </button>
            </div>
            <div class="table-cell" style="color: #ffa500">
          ...
          <div class="table-row footer">
            <div class="table-cell">
              <div class="allproduct" style="color: #ffa500" @click="toPageProduct()">继续购物</div>
            </div>
            <div class="table-cell">已选择<span style="color: #ffa500;">{{ totalNum }}</span>件</div>
            <div class="table-cell"></div><div class="table-cell"></div><div class="table-cell"></div>
            <div class="table-cell">
              总价:￥{{ totalPrice }}
            </div>
            <div class="table-cell">
              <button class="checkout-btn" @click="toPagePayment()">去结账</button>
            </div>
          </div>
        </div>
      </div>
    ...
    methods: {
      ...
      async increment(item) {
        try {
          ...
          item.sNum++;
          // 使用 Vue.set 确保响应性
          this.shoppingList[index] = { ...this.shoppingList[index], sNum: item.sNum };
