    ...
    <div>
        ...
        <div class="horizontal-list-container">
            <ul class="horizontal-list">
                <li>分类:</li>
                <li :class="sType === '全部' ? 'sType': null" @click="toType('全部'); getProduct(0)">全部</li>
                <li v-for="item of typeList" :key="item.tid" :class="sType === item.tname ? 'sType': null" @click="toType(item.tname); getProduct(item.tid)">{{ item.tname }}</li>
            </ul>
        </div>
        <!-- 分类栏END -->
        <!-- 商品展示区域 -->
    ...
    methods: {
        ...
        async getProduct(tid){
            if(this.getSearch === ''){
                ...
                const response = await axios.get('/api/getProductByKeyword?keyword=' + keyword);
                this.productList = response.data;
