<template>
  <div id="body_div">
    ...
    <div id="body_div_right">
      ...
      <div v-if="UserInfoWindow === 'secure'" id="show_1">
        <div style="font-weight: bold; font-size: 20px; margin: 20px">
          |登录方式
        </div>
        <ul>
      ...
      <div v-if="UserInfoWindow === 'info'" id="show_2">
        ...
        <div id="show_2_div">
          ...
          <div class="show_2_div_body">
            ...
            <input
              type="text"
              id="nickname_input"
              v-model.lazy="UserInfo.unickName"
            />
          </div>
          <div class="show_2_div_body">
            <text>性别</text>
        ...
        <div id="show_2_div_btn">
          <button id="modify_info_btn" @click="modifyInfo()">
            <span id="info_btn_text">修改个人信息</span>
          </button>
        </div>
      </div>
      <!-- 个人信息END -->
      ...
      <div v-else-if="UserInfoWindow === 'address'" id="show_3">
        ...
        <div id="shou_3_overflow_scroll">
          <ul id="show_3_div" v-for="addres of address" :key="addres.aid">
            <li>
              姓名：{{ addres.aname }}<br />
              手机：{{ addres.aphonenNmber }}<br />
              地址：{{ addres.atext }}<br />
              <div style="margin-top: 15px">
                <button @click="deleteAddress(addres.aid)">删除</button>
                <button @click="toUpdateAddress(addres, 'moSite')">修改</button
                ><br />
              </div>
            </li>
          </ul>
        ...
        <button
          id="add_address_btn"
          class="add_address_btn_class"
          @click="toUserFunction('addSite')"
        >
          添加地址
        </button>
      </div>
  ...
  <div v-if="UserFunction === 'phone'" id="find_phone_div">
    <div @click="toUserFunction('')">
      <img src="../assets/close.png" class="close" id="close_phone_btn" />
    </div>
  ...
  <div v-else-if="UserFunction === 'email'" id="find_email_div">
    <div @click="toUserFunction('')">
      <img src="../assets/close.png" class="close" id="close_email_btn" />
    </div>
  ...
  <div v-else-if="UserFunction === 'password'" id="modify_pwd_div">
    <div @click="toUserFunction('')">
      <img src="../assets/close.png" class="close" id="close_pwd_btn" />
    </div>
  ...
  <!-- 添加收货地址 -->
  <div
    v-else-if="UserFunction === 'addSite'"
    class="add_address_div"
    id="add_address_div"
  >
    <div @click="toUserFunction('')">
      <img src="../assets/close.png" class="close" id="close_address_btn" />
    </div>
    ...
    <div class="add_address_div_content">
      ...
      <input
        type="text"
        placeholder="请输入手机号"
        v-model="newAddress.aphonenNmber"
        id="aphone_input"
      />
  ...
  <!-- 修改收货地址 -->
  <div
    v-else-if="UserFunction === 'moSite'"
    class="add_address_div"
    id="add_address_div_x"
  >
    <div @click="toUserFunction('')">
      <img src="../assets/close.png" class="close" id="close_address_btn_x" />
    </div>
...
export default {
  ...
  methods: {
    async modifyInfo() {
      ...
      if (this.UserInfo.unickName !== null && this.UserInfo.unickName !== "") {
        const response = await axios.get(
          "/api/updateUserNickName?unickname=" +
            this.UserInfo.unickName +
            "&uname=" +
            this.getUserName
        );
        if (response.data == 1) {
    ...
    async getUserInfo() {
      const response = await axios.get(
        "/api/getUserInfo?uname=" + this.getUserName
      );
    ...
    async getUserAddress() {
      const response = await axios.get(
        "/api/getAddressByUName?uname=" + this.getUserName
      );
      this.address = response.data;
    },
    async phoneChange() {
      if(this.newUPhoneNmber != null && this.newUPhoneNmber != ""){
          const response = await axios.get(
            "/api/phoneChange?uname=" +
              this.getUserName +
              "&phone=" +
              this.newUPhoneNmber
          );
    ...
    async emailChange() {
      if(this.newUEmail != null && this.newUEmail != ""){
          const response = await axios.get(
            "/api/emailChange?uname=" +
              this.getUserName +
              "&email=" +
              this.newUEmail
          );
    ...
    async pwdChange() {
      if(this.reNewPWD != null && this.reNewPWD != "" && this.newPWD != null && this.newPWD != ""){
